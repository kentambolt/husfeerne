...
> **About the site:** A static website on GitHub Pages, with no database and no cookies. It exists in four languages – Danish, English, German and Ukrainian – each with its own pages: Danish at `husfeerne.dk/`, the other three under `/en/`, `/de/` and `/uk/` with translated addresses (e.g. `/en/home-cleaning-aalborg.html`). The flag selector at the top switches to the page in the chosen language, and the choice is remembered in the browser. All four versions are tied together with `hreflang`, so Google shows the right one for each search. Forms are sent via Formspree. Our internal documents live at `/cherpak/` – a shared archive for the three of us in Danish, English and Russian. It is not secret, but the pages are marked `noindex` so they do not compete with the sales pages in Google. The sources live in `cherpak/_src/` and are built with `python cherpak/build.py`; sources and print files are not published. The English, German and Ukrainian pages are generated by `build_lang.py` from the Danish pages and the translations in `i18n/tr/*.json` – always edit the Danish page and the JSON files and run `python build_lang.py build`; never edit `/en/`, `/de/` and `/uk/` directly.
...
- [x] **Our own photos in place.** Twelve pictures of us at work are used on the front page and the sub-pages: team photo with the logo on the shirts (hero and "Meet the team"), cushions ("Our approach"), sofa, stairwell, window and car cleaning (front-page cards), lamp (deep cleaning), vacuuming and kitchen (home cleaning), oven and bathroom (move-out cleaning), floor (business), stairwell and windows on their own pages. The compressed versions have keyword filenames (e.g. `privat-rengoering-aalborg-stoevsugning.jpg`, `trappevask-aalborg-opgang.jpg`) plus `husfeerne-team.jpg`, all with `-560`/`-800` variants for mobile; the originals live in `raw_photos/` and are not published. No stock photos remain.
- [x] **Logo in place.** The site uses `husfeerne-logo.png` – a web-optimised version 440 px wide (10 KB, 256 colours), plenty for the 148×66 px the logo is shown at. The high-resolution original is `husfeerne-logo-print.png` (1880 px, 479 KB) for print, business cards and car decals; the website does not load it. The file `husfeerne-logo.jpg` is no longer used and can be deleted.
...
- **Speed**: the hero photo is preloaded with `fetchpriority="high"`; every photo has `srcset` at 560/800 (and 840/1040 for the team photo), so mobile downloads a quarter of the data; app.js loads with `defer`; the fonts (Cormorant Garamond, Lato and Source Sans 3 for Cyrillic) are self-hosted from `/fonts/` via `fonts.css` with `font-display:swap`, and the two most important files are preloaded – no requests to Google Fonts, so no third party for fonts in the privacy policy either.
...
- **Four languages, each with its own pages**: the front page and the six service pages exist in Danish, English, German and Ukrainian – 28 pages in total, all in `sitemap.xml` with `hreflang` alternates. Each language version has its own address with keywords in that language, its own title, description, headings, FAQ and structured data, and `x-default` points to Danish. The forms still submit the Danish option labels, so we read the same thing whatever language the customer used. The Ukrainian pages use the Source Sans 3 typeface for body text because Lato has no Cyrillic characters.
- **Deliberately not done**: review schema (we have no reviews yet – it must never be invented).
...
- [x] **Multilingual SEO.** All four languages have their own addresses (`/`, `/en/`, `/de/`, `/uk/`) linked with `hreflang`, so Google can show the English page for an English search. Watch in Search Console which languages actually bring traffic – and make text changes in the Danish page and `i18n/tr/`, so the other three languages follow at the next `python build_lang.py build`.
