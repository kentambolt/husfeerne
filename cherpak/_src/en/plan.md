...
- [x] **Our own photos in place.** Twelve pictures of us at work are used on the front page and the sub-pages: team photo with the logo on the shirts (hero and "Meet the team"), cushions ("Our approach"), sofa, stairwell, window and car cleaning (front-page cards), lamp (deep cleaning), vacuuming and kitchen (home cleaning), oven and bathroom (move-out cleaning), floor (business), stairwell and windows on their own pages. The compressed versions are `foto-*.jpg` and `husfeerne-team.jpg` with `-560`/`-800` variants for mobile; the originals live in `raw_photos/` and are not published. No stock photos remain.
...
## Phase 0c · The SEO setup as it stands

The site is set up to rank for cleaning of private homes and offices in Aalborg. This is in place today:

- **Titles and descriptions** are under 60/160 characters and carry the words people actually search for: the front page "Rengøring Aalborg – privat og erhverv", the home-cleaning page "Privat rengøringshjælp i Aalborg", the business page "Erhvervsrengøring og kontorrengøring i Aalborg". The words *rengøringshjælp*, *kontorrengøring* and *rengøringsfirma* now appear in headings and body text – they were missing entirely before.
- **Aalborg districts** are named naturally in the coverage section, FAQ and price boxes: Hasseris, Vejgaard, Gug, Kærby, Skalborg, Vestbyen, Aalborg Øst, Nørresundby, Svenstrup, Klarup, Vodskov, Vadum, Nibe. No per-district pages – Google would penalise those as doorway pages.
- **Structured data**: the business has one `@id` (`https://husfeerne.dk/#organization`) on every page, so Google sees one company rather than seven; the front-page service catalogue points to the sub-pages; the FAQPage schema is generated from the visible FAQ and matches it word for word (a requirement). The coordinates in the schema are set to Akvavitvej (57.0547, 9.9085) – **check them against Google Maps**; they were calculated, not looked up.
- **Technical**: `<main>` landmark, breadcrumbs, skip link and aria on the menu; `404.html` in the site's design; `sitemap.xml` with only the seven indexable pages; real favicon files (`favicon.svg`, `favicon.ico`, `favicon-48.png`, `apple-touch-icon.png`) so Google shows the leaf in results; canonical links to `/` instead of `index.html`; internal links with "… i Aalborg" as anchor text; phone and e-mail clickable in the contact card.
- **Speed**: the hero photo is preloaded with `fetchpriority="high"`; every photo has `srcset` at 560/800 (and 840/1040 for the team photo), so mobile downloads a quarter of the data; app.js loads with `defer`; the Google Fonts URL only fetches the weights in use.
- **Share images**: `husfeerne-og.jpg` (1200×628) is used when sharing on Facebook/LinkedIn; stairwells and window cleaning have their own cropped versions.
- **Deliberately not done**: hreflang (needs separate URLs per language), review schema (we have no reviews yet – it must never be invented), self-hosted fonts (needs a download), renaming `foto-*.jpg` to keyword names (small gain, lots of churn).

> **Two statements we must be able to stand behind:** the FAQ now says Husfeerne is a cleaning company with *its own employed* staff – not a platform. And the three old stock photos (stairwell, windows, car) were deleted because they showed other companies' logos; our own photos are in their place.
